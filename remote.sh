...
CLEAN_RESULT_CMD="cd $PROJECT_DIR; rm data/*.txt; rm *.rdb; rm -rf data/workers; rm -rf $JSTORM_DIR/data/*; rm -rf /root/zookeeper/version-2/*; rm -rf $PROJECT_DIR/$JSTORM_DIR/data/*"
...
ADD_ZOO="echo \"heron.statemgr.connection.string: zookeeper-node01:2181\" >> ~/.heron/conf/standalone/statemgr.yaml"
...
function benchmarkLoop (){
    runAllServers "${PULL_GIT}"
        ...
        while true; do
            ...
            scp ${SSH_USER}@redisdo:~/stream-benchmarking/dump.rdb ${PROJECT_DIR}/resultLogs/${ALGORITHM}/result/$(date +%Y-%m-%d_%H%M%S)_TPS_${TPS}/dump.rdb
