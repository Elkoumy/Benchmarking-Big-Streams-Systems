...
#        $KAFKA_DIR/bin/kafka-topics.sh --create --zookeeper "$ZK_CONNECTIONS" --replication-factor 3 --partitions 3 --topic ${TOPIC}
         $KAFKA_DIR/bin/kafka-topics.sh --create --zookeeper "$ZK_CONNECTIONS" --replication-factor 1 --partitions 1 --topic ${TOPIC}
    else
    ...
    then
        ...
        $KAFKA_DIR/bin/kafka-topics.sh --create --zookeeper "$ZK_CONNECTIONS" --replication-factor 1 --partitions 1 --topic purchases
    else
        echo "Kafka topic $TOPIC already exists"
    fi
    ...
    then
        ...
        $KAFKA_DIR/bin/kafka-topics.sh --create --zookeeper "$ZK_CONNECTIONS" --replication-factor 1 --partitions 1 --topic ads
    else
        echo "Kafka topic $TOPIC already exists"
    fi
}
...
#    "$SPARK_DIR/bin/spark-submit" --master spark://${SPARK_MASTER_HOST}:7077 --class spark.benchmark.StreamSqlBenchQueriesSpark /root/stream-benchmarking/spark-java-benchmarks/target/spark-java-benchmarks-0.1.0.jar "$CONF_FILE" &
     "$SPARK_DIR/bin/spark-submit"  --class spark.benchmark.StreamSqlBenchQueriesSpark /root/stream-benchmarking/spark-java-benchmarks/target/spark-java-benchmarks-0.1.0.jar &
    sleep 5
  elif [ "STOP_SPARK_PROCESSING" = "$OPERATION" ];
  then
#    stop_if_needed spark.benchmark.KafkaRedisAdvertisingStream "Spark Client Process"
    stop_if_needed spark.benchmark.StreamSqlBenchQueriesSpark "Spark Client Process"
